...
class adminsSearchModel extends My_Model
{
    ...
    function __construct(){
        ...
        $this->my_column_search=array('firstname','lastname','username','gender','email');
        $this->my_order=array('firstname'=>'asc');
    }//end

    function fetch_data(){
        $data=array();
        $list=$this->admin_result_builder();
        foreach($list as $row){
            $subdata=array();
            //$subdata[]=$row->userid;
        ...
        return $this->admin_json_builder($data);
    }//end
    //
