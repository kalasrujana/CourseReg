...
<html lang="en">
   <head>
      <title>Edit Administrators List</title>
      <meta charset="utf-8">
   ...
   <body background="<?php echo base_url('images/bg.jpg'); ?>">
      ...
      <div class="container">
          <h1 style="color:white">Edit Administrators List</h1>
      ...
      <h4 class="modal-title">Update Administrator Details</h4>
      </div>
      ...
      <label>Phone</label>
      <input type="text" name="phone" id="phone" maxlength="10" class="form-control" pattern="^(?!(\d)\1{9})(?!0123456789|1234567890|0987654321|9876543210)\d{10}$" title="enter 10 digit phone number"/><br>
      <label>Address</label>
      ...
      <script>
         ...
         $(document).ready(function(){
             $('#example').DataTable({
                 ...
                 "pageLength":5,
                 "ajax":{
                     "url":"<?php echo site_url()?>/AdminsSearchCon/fetch",
                     "type": "POST"
                 },
                 "columnDefs":[{
                     "targets":[-1],
           ...
           //delete admin from the datatable
           $(document).on('click','.delete',function(){
             ...
             if(confirm("Are you sure you want to delete this administrator?"))
             {
                 ...
                 $.ajax({
                      url:"<?php echo site_url()?>/AdminsSearchCon/deleteStudent",
                      method:"POST",
           ...
           //userd is used as input ie., this will have the useridd which is used while quering 
            $.ajax({
                      url:"<?php echo site_url()?>/AdminsSearchCon/fetch_single_user",
                      method:"GET",
                      data:{userd:useridd},
                      datatype:"json",
                      success:function(data){
                         ...
                         $('#userModal').modal('show');
                          $('#userModal').find('.modal-title').text("Edit Admin Data");
         ...
         $(document).on('submit', '#user_form', function(event){  
                ...
                {  
                     $.ajax({  
                          url:"<?php echo site_url()?>/AdminsSearchCon/user_action",  
                          method:'POST',  
                ...
                {  
                     alert("FirstName,Last Name and Email fields are Required");  
                }  
           });  
